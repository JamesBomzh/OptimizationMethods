...
def grad(x, y, num, power):  # гардиент функции
...
gradf = grad(x, y, u0, 1) + grad(y, x, u0, 1)  # Шаг 2: находим gardJ(u0)
gradNorm = mp.sqrt((grad(x, y, u0, 1)) ** 2 + (
    grad(y, x, u0, 1)) ** 2)  # Находим ||gard J(u0)|| для проверки критерия остановки метода

while (gradNorm >= eps):  # Пока критерий остановки метода ||gard J(u0)|| < eps не выполнен, делаем следубщие шаги:
    u1 = u0 - alpha * gradf  # Шаг 3: Полагаем, что u1 = u0 - alpha*gardJ(u0)
    Ju1 = grad(x, y, u1, 0)
    Ju0 = grad(x, y, u0, 0)
